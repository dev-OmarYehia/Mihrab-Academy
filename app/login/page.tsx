  ...
  return (
    <div className="min-h-screen bg-[#0f1a14] flex items-center justify-center px-6">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}
        ...
        <h1 className="text-3xl font-light text-white mb-1">Welcome Back</h1>
        <p className="text-[#a8c5a0]/60 text-sm mb-8">Sign in to your account</p>
        {error && <div className="bg-red-500/10 border border-red-500/30 text-red-300 text-sm px-4 py-3 rounded-xl mb-6">{error}</div>}
        <div className="space-y-4">
          {[{ label: "Email", name: "email", type: "email", placeholder: "omar@example.com" },
        ...
        <p className="text-center text-[#a8c5a0]/40 text-xs mt-6">
          Don&apos;t have an account? <Link href="/signup" className="text-[#c9a96e] hover:underline">Sign up</Link>
        </p>
      </motion.div>
    </div>
  );
